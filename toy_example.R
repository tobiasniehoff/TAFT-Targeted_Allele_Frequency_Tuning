...
for(gen.i in 1:n.generations){
  ...
  for(ind.i in 1:pop.size){
    ...
    # The function 'get.individual.loc()' is not available in MoBPS version >=1.13.1
    ...
    get.individual.loc <- function(population, database = NULL, gen = NULL, cohorts = NULL){
      ...
      for (row in 1:nrow(database)) {
        ...
        data[(before + 1):(before + nanimals), ] <- cbind(database[row, 
                                                                   1], database[row, 2], database[row, 3]:database[row, 
                                                                                                                   4])
        rown[(before + 1):(before + nanimals)] <- paste(if (database[row, 
                                                                     2] == 1) 
          "M"
          else "F", database[row, 3]:database[row, 4], "_", database[row, 
                                                                     1], sep = "")
